...
export default {
  ...
  computed: {
    ...
    newChildNode(){ return store.state.explorer.newChildNode; },
    rootNode(){ return store.getters['explorer/rootNode']; },
    selectedNode(){ return store.getters['explorer/selectedNode']; },
  },
  methods: {
    toggleExplorer(doOpen){ store.dispatch('data/toggleExplorer', doOpen); },
    newItem(isFolder){ explorerSvc.newItem(isFolder); },
    deleteItem(){ explorerSvc.deleteItem(); },
    editItem() {
      const node = this.selectedNode;
      if (!node.isTrash) { // Trash cannot be edited
        store.commit('explorer/setEditingId', node.item.id);
      }
    },
  },
  created() {
    this.$watch( // TODO ne devrait pas etre la mais directement dans workspaceSvc
      () => store.getters['fileInfo/current'].id,
      (currentFileId) => {
