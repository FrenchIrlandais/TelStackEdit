...
const computedLayoutSetting = key => ({
  get() {
    return store.getters['data/layoutState'][key];
  },
  set(value) {
...
class DynamicClassApplier {
  ...
  clean() {
    ...
    })(); // execute directement la lambda function,
    // je ne sais pas si cest necessaire de passer par une lambda function
    // ça depende ce que "this.classApplier" reference...
  }
}
...
export default {
  ...
  computed: {
    type(){ return store.state.findReplace.type; },
    lastOpen(){ return store.state.findReplace.lastOpen; },
  ...
  methods: {
    ...
    find(mode = 'forward') {
      ...
      if (this.selectedClassApplier) {
        ...
        selectionMgr.updateCursorCoordinates(el.parentNode.clientHeight);
  ...
  mounted() {
    ...
    watch(() => this.findText, this.debouncedHighlightOccurrences);
    watch(() => this.findCaseSensitive, this.debouncedHighlightOccurrences);
    watch(() => this.findUseRegexp, this.debouncedHighlightOccurrences);
    ...
    watch(() => this.lastOpen, () => {
      const elt = el.querySelector(`.find-replace__text-input--${this.type}`);
      elt.focus();
    ...
    this.onFocusIn = () => el.contains(document.activeElement) ||
