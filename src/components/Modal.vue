  ...
  <div class="modal" v-if="config" @keydown.esc.stop="onEscape" @keydown.tab="onTab" @focusin="onFocusInOut" @focusout="onFocusInOut">
    ...
    <modal-inner v-else aria-label="Dialog">
      ...
      <div class="modal__button-bar">
        <button class="button" v-if="simpleModal.rejectText" @click="onClickReject()" :disabled="isDisabled">{{simpleModal.rejectText}}</button>
        <button class="button button--resolve" v-if="simpleModal.resolveText" @click="onClickReject()" :disabled="isDisabled">{{simpleModal.resolveText}}</button>
      </div>
    </modal-inner>
  </div>
</template>
...
import SupportedLanguagesModal from './modals/SupportedLanguagesModal';
...
export default {
  ...
  data: () => ({
    /* loaded is false when a new pop-up appear for some milliseconds in order
    * to prevent event propagation like keydown.enter ...
    * I don't know how to do it properly */
    loaded: true,
  }),
  computed: {
    isDisabled(){ return !this.loaded; },
