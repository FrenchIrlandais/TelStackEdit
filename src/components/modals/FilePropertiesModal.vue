...
export default {
  ...
  computed: {
    ...
    tab: {
      get() {
        return store.getters['data/layoutState'].filePropertiesTab;
      },
      set(value) {
  ...
  created() {
    const properties = store.getters['fileContent/getProperties'];
    this.setYamlProperties(properties);
    if (this.tab !== 'yaml') {
      this.setSimpleTab();
    }
  },
  methods: {
    ...
    resolve() {
      ...
      if (this.error) {
        ...
        store.commit('fileContent/setProperties', utils.sanitizeText(this.yamlProperties));
        this.config.resolve();
      }
    },
  },
