...
export default {
  ...
  */
  async applyTemplate(fileInfo, fileContentText, fileProperties, template = {
    value: '{{{files.0.content.text}}}',
    helpers: '',
  }, pdf = false) {
    const properties = utils.computeProperties(fileProperties);
    const options = extensionSvc.getOptions(properties);
    const converter = markdownConversionSvc.createConverter(options, true);
    const parsingCtx = markdownConversionSvc.parseSections(converter, fileContentText);
  ...
  */
  async exportToDisk(fileInfo, fileContentText, fileProperties, type, template) {
    const html = await this.applyTemplate(fileInfo, fileContentText, fileProperties, template);
    const blob = new Blob([html], {
      type: 'text/plain;charset=utf-8',
    });
    FileSaver.saveAs(blob, `${fileInfo.name}.${type}`);
  },
};
