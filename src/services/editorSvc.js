...
import Prism from './prismWrapperSvc';
...
const editorSvc = Object.assign(new Vue(), editorSvcDiscussions, editorSvcUtils, {
  ...
  */
  initPrism() {
    ...
    options.insideFences = markdownConversionSvc.defaultOptions.insideFences;
  ...
  */
  initClEditor() {
    ...
    const options = {
      ...
      getCursorFocusRatio: () => {
        if (store.getters['data/layoutState'].focusMode) {
          return 1;
        }
        return 0.15;
      },
    ...
    this.conversionCtx.htmlSectionDiff.forEach((item) => {
      for (let i = 0; i < item[1].length; i += 1) {
        ...
        if (item[0] === 0) {
          ...
          loadingImages = loadingImages.concat(Array.prototype.slice.call(sectionPreviewElt.getElementsByTagName('img')));
  ...
  */
  saveFileState: allowDebounce(() => {
    const scrollPosition = editorSvc.getScrollPosition() ||
      store.getters['fileInfo/current'].scrollPosition;
    store.dispatch('fileInfo/patchCurrent', {
  ...
  */
  init(editorElt, previewElt, tocElt) {
    ...
    this.editorElt.parentNode.addEventListener('scroll', () => this.saveFileState(true));
    this.previewElt.parentNode.addEventListener('scroll', () => this.saveFileState(true));
    ...
    */
    ...
    let lastProperties;
    store.watch(
      () => store.getters['fileContent/changeTrigger'],
      () => {
        const content = { // TODO, quesque ça fait ça au final ?
          id: 'yolo', // TODO: on va avoir un pb avec les ID
        ...
        if (content.properties !== lastProperties) {
          ...
          const computedProperties = utils.computeProperties(content.properties); // TODO, move this? or what ?
          const options = extensionSvc.getOptions(computedProperties);
    ...
    store.watch(
      () => store.getters['fileContent/isEditable'],
